...
def main(write_to):
    ...
    for module_name in sys.argv[1:]:
        try:
            ...
            print ("Couldn't import: %s. %s" % (module_name))
    ...
    if '-v' in sys.argv:
        ...
        STDOUT_ONLY = True
            ...
            for module_name in sys.argv[1:]:
                for line in file_lines:
                    if line.find('--- import %s ' % module_name) != -1:
                        print ('"%s" already exists in %s. Skipping...' % (module_name, PYDICTION_DICT))
                        sys.argv.remove(module_name)
                        break
            f.close()
            ...
            if os.path.exists(PYDICTION_DICT_BACKUP):
                ...
                if (answer):
                    print ("Backing up old dictionary to: %s" % PYDICTION_DICT_BACKUP)
                    try:
                        shutil.copyfile(PYDICTION_DICT, PYDICTION_DICT_BACKUP)
                    except IOError:
                        print ("Couldn't back up %s. %s" % (PYDICTION_DICT,))
                else:
                    print ("Skipping backup...")

                print ('Appending to: "%s"' % PYDICTION_DICT)
            else:
                print ("Backing up current %s to %s" % (PYDICTION_DICT, PYDICTION_DICT_BACKUP))
                try:
                    shutil.copyfile(PYDICTION_DICT, PYDICTION_DICT_BACKUP)
                except IOError:
                    print ("Couldn't back up %s. %s" % (PYDICTION_DICT))
