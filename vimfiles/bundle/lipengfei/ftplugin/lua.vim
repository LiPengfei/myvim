...
" 
if has("gui_running")
    "map changed by lipengfei 2013/03/12
    nmap    <buffer>  <silent>  <C-F3>        :call Lua_SyntaxCheck()<CR>:redraw!<CR>:call Lua_SyntaxCheckMsg()<CR>
    imap    <buffer>  <silent>  <C-F3>        <Esc>:call Lua_SyntaxCheck()<CR>:redraw!<CR>:call Lua_SyntaxCheckMsg()<CR>
    " 
    " <C-C> seems to be essential here:
    "
    map    <buffer>  <silent>  <C-F4>        :call Lua_Run()<CR>
    imap    <buffer>  <silent>  <C-F4>        <Esc>:call Lua_Run()<CR>
    "
    map    <buffer>  <silent>  <S-F4>             :call Lua_Arguments()<CR>
    imap    <buffer>  <silent>  <S-F4>        <Esc>:call Lua_Arguments()<CR>
...
:nnoremap <buffer> <leader>sf :vsp D:\Vim\vimfiles\ftplugin\lua.vim<CR>
:inoremap <buffer> <leader>sf <Esc>:vsp D:\Vim\vimfiles\ftplugin\lua.vim<CR>

setlocal includeexpr=substitute(v:fname,'\\\\\\\\script','script','g') 
